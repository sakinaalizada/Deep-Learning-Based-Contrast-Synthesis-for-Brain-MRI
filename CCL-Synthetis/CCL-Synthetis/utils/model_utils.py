...
class modelObj:
    def __init__(self, cfg, kernel_init=None):
        ...
        self.num_output_channel = cfg.num_output_channel
        if kernel_init == None:
            self.kernel_init = tf.keras.initializers.HeNormal(seed=1)
        else:
            self.kernel_init = kernel_init
        
    def encoder_block_contract(self, block_input, num_fts, pool_flag=True, block_name=1, act_name='relu'):        
        ...
        down = Activation(act_name ,name = 'enc_act1_'+ str(block_name))(down)
        ...
        down = Activation(act_name , name = 'enc_act2_'+ str(block_name ))(down)
        return down

    def decoder_block_expand(self, block_input, numFts, concat_block, upsample_flag=True, block_name=1, act_name = 'relu'):
        ...
        if upsample_flag:
            ...
            block_input = Activation(act_name , name = 'dec_upsamp_act_'+ str(block_name ))(block_input)
        ...
        up = Activation(act_name , name = 'dec_act1_'+ str(block_name ))(up)
        ...
        up = Activation(act_name , name = 'dec_act2_'+ str(block_name))(up)
        return up
    ...
    def synth_unet(self, act_name = 'relu'):
        'Network for downstream synthesis tasks'
        no_filters = self.no_filters  
        inputs = Input((self.img_size_x, self.img_size_y, self.num_channels))
        ...
        # Encoder network
        ###################################
        enc_c1 = self.encoder_block_contract(inputs, no_filters[1], pool_flag=False, block_name=1, act_name = act_name)
        enc_c2 = self.encoder_block_contract(enc_c1, no_filters[2], block_name=2, act_name = act_name)
        enc_c3 = self.encoder_block_contract(enc_c2, no_filters[3], block_name=3, act_name = act_name)
        enc_c4 = self.encoder_block_contract(enc_c3, no_filters[4], block_name=4, act_name = act_name)
        enc_c5 = self.encoder_block_contract(enc_c4, no_filters[5], block_name=5, act_name = act_name)
        enc_c6 = self.encoder_block_contract(enc_c5, no_filters[5], block_name=6, act_name = act_name)

        ###################################
        # Decoder network - Upsampling Path
        ###################################
        dec_c5 = self.decoder_block_expand(enc_c6, no_filters[5], enc_c5, block_name=5, act_name = act_name)        
        dec_c4 = self.decoder_block_expand(dec_c5, no_filters[4], enc_c4, block_name=4, act_name = act_name)
        dec_c3 = self.decoder_block_expand(dec_c4, no_filters[3], enc_c3, block_name=3, act_name = act_name)
        dec_c2 = self.decoder_block_expand(dec_c3, no_filters[2], enc_c2, block_name=2, act_name = act_name)
        dec_c1 = self.decoder_block_expand(dec_c2, no_filters[1], enc_c1, block_name=1, act_name = act_name)
            
        model_op = Conv2D(16,3,padding='same', kernel_initializer=self.kernel_init)(dec_c1)
        model_op = GroupNormalization(groups = 4)(model_op)
        seg_op2 = Activation(act_name)(model_op)
        seg_op = Conv2D(self.num_output_channel , 1, name='seg_layer', padding='same',use_bias=False, kernel_initializer=self.kernel_init)(seg_op2) #use_bias=False
        model = Model(inputs=[inputs], outputs=[seg_op])
        return model
    
    def vgg16_feature_extractor(self):

        # Create a VGG16 model to compare features for perceptual loss

        xDim = self.img_size_x

        yDim = self.img_size_y

        vgg16 = tf.keras.applications.vgg16.VGG16(include_top=False, weights='imagenet', input_tensor=None) 

        # Create a new VGG model to compare relu output
